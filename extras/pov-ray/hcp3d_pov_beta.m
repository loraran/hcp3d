...
% Rotating atomcut1 and atomcut2 30° around Z-axis so it can be on the same position as alpha
inclangle = -30;
inclanglerad = (-inclangle)*(pi/180);
X1 = atomcut1(:,1)*cos(inclanglerad) - atomcut1(:,2)*sin(inclanglerad);
Y1 = atomcut1(:,2)*cos(inclanglerad) + atomcut1(:,1)*sin(inclanglerad);
Z1 = atomcut1(:,3);
atomcut1 = [X1 Y1 Z1 atomcut1(:,4)];
X2 = atomcut2(:,1)*cos(inclanglerad) - atomcut2(:,2)*sin(inclanglerad);
Y2 = atomcut2(:,2)*cos(inclanglerad) + atomcut2(:,1)*sin(inclanglerad);
Z2 = atomcut2(:,3);
atomcut2 = [X2 Y2 Z2 atomcut2(:,4)];
% Visualization
figure;
scatter3(atomcut1(:,1),atomcut1(:,2),atomcut1(:,3),15,[0 0 1],'filled'); hold on;
scatter3(atomcut2(:,1),atomcut2(:,2),atomcut2(:,3),15,[1 0 0],'filled'); hold off;
xlabel('x'); ylabel('y'); zlabel('z');
axis equal;
view(90,0)
pause

cont = 0; % Filtering Z (-5:5) and X (-1:1) for atomcut1
...
cont = 0; % Filtering Z (-5:5) and X (-1:1) for atomcut2
...
% subplot(1,2,2); scatter3(atompov2(:,1),atompov2(:,2),atompov2(:,3),15,[1 0 0],'filled');
% xlabel('x'); ylabel('y'); zlabel('z'); axis equal; view(110,20);
% pause

figure;
scatter3(atompov1(:,1),atompov1(:,2),atompov1(:,3),25,[0 0 1],'filled'); hold on;
scatter3(atompov2(:,1),atompov2(:,2),atompov2(:,3),25,[1 0 0],'filled'); hold off;
xlabel('x'); ylabel('y'); zlabel('z');
...
path = strcat('C:/Users/Loren/Desktop/hcp3d_plots/POV-Ray/beta - ',datestr(clock,'dd-mmm-yyyy HH.MM.SS')); mkdir(path);
