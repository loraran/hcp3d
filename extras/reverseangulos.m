...
inclangle = input('Insert rotation angle(s) [DEG]: '); %degrees
cont = 1;
for z = -18:0.001:0
    % alpha plane [REDE ANTIGA SEP/2018]
    P = [ -0.8002 -6.171e-16 0 ; -0.2667 -0.308 z ; 0.8002 -0.308 z ];
    % Find plane equation : ax + by + cz + d = 0
    D = det(P);
    A = P; A(:,1) = 1; dA = det(A);
    B = P; B(:,2) = 1; dB = det(B);
    ...
    b = ((-1)*d/D)*dB;
    c = ((-1)*d/D)*dC;
    % Calculate corresponding angle of plane inclination
    modP = sqrt((a^2)+(b^2)+(c^2));
    angle = ((acos(c/modP))-(pi/2))*(180/pi); %degrees
    if round(angle) == inclangle
        angles(1,cont) = angle;
        angles(2,cont) = z;
        cont = cont + 1;
%         fprintf('z = %g   angle = %g\n',z,angle);
    ...
    compare = bsxfun(@minus,angles,inclangle);
%     disp(min(abs(compare)));
