function [mgfourier] = hcp3d_fftXh(fmg,sepDist,clatt,inclangle,atomname,atomcalc,path,coordstep)
...
%         hold off;
        ...
        fprintf('  (h k l) planes = (H %g %g) ...\n',kLoc,lLoc);
        
        % Saving mgfourier to .txt file (comment to disable)
        fftname = strcat(atomname,'_ang',num2str(inclangle),'_j',num2str(kLoc),num2str(lLoc),'.txt');
...
%         fclose('all');
    ...
    if lLoc == 6
        subplot(1,3,[2 3])
        legend('H00','H01','H02','H03','H04','H05','H06')
    else
    end;
end;
